    ...
    private function parse_response($response) {
        ...
        if ($this->curl->errno !== CURLE_OK) {
            if ($this->curl->errno == CURLE_OPERATION_TIMEOUTED) {
                ...
                throw new \Exception("Error calling levitate API: " . $error . " (Called URL $url)");
            }
        }
        ...
        if ($info['http_code'] == 200) {
            if (self::is_content_type_json($info['content_type'])) {
                ...
                if (json_last_error() == 0) {
                    if (empty($data) or !is_object($data)) {
                        throw new \Exception("Empty response returned from levitate API (Called URL $url)");
                    }
                    return $data;
                } else {
                    $error = json_last_error_msg();
                    throw new \Exception("JSON error parsing response from levitate API: $error (Called URL $url). Response: $response");
                }
            } else {
                return $response;
            }
            ...
            $message = "Unexpected response from levitate API. Received " . $info['http_code'];
