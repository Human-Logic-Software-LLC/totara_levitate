...
use contentmarketplace_levitate\api;
use contentmarketplace_levitate\config_session_storage;
use contentmarketplace_levitate\string_manager;
...
final class setup_form extends \totara_form\form {
    ...
    protected function definition() {
        ...
        $s1 = $wizard->add_stage(new setup_wizard_stage('stage_one', get_string('account', 'contentmarketplace_levitate')));
        ...
        $s1->add(new static_html(
            'account',
            null,
            $OUTPUT->render_from_template('contentmarketplace_levitate/account', $data)
        ));

        $s2 = $wizard->add_stage(new setup_wizard_stage('stage_two', get_string('content_settings', 'contentmarketplace_levitate')));
        $s2->add(new static_html(
            'creatorsdesc',
            '',
            get_string('content_settings_description', 'contentmarketplace_levitate')
        ));
        $s2->add(new radios(
            'creators',
            get_string('content_creators', 'contentmarketplace_levitate'),
            array(
                'all' => get_string('all_content', 'contentmarketplace_levitate', $this->parameters['courses_all'] ?? null),
                'subscribed' => get_string('subscribed_content', 'contentmarketplace_levitate', $this->parameters['courses_subscribed'] ?? null),
                'collection' => get_string('collection_content', 'contentmarketplace_levitate', $this->parameters['courses_collection'] ?? null),
            )
        ))->add_help_button('content_creators', 'contentmarketplace_levitate');

        $payperseat = new hidden('pay_per_seat', PARAM_INT);
        $s2->add($payperseat);

        $wizard->set_submit_label(get_string('saveandexplorelevitate', 'contentmarketplace_levitate'));
        $wizard->finalise();
    }
    ...
    private static function format_account_data(&$data) {
        ...
        if (!empty($data->plan->type)) {
            $data->plan->type = get_string('levitateplantype', 'contentmarketplace_levitate', \core_text::strtotitle($data->plan->type));
        }

        $courses = get_config('contentmarketplace_levitate', 'learning_objects_subscribed');
        if (!empty($courses)) {
            $data->plan->type .= ' '.get_string(
                'courses_amount_label',
                'contentmarketplace_levitate',
