...
namespace contentmarketplace_levitate\testing;

use coding_exception;
use contentmarketplace_levitate\api;
use contentmarketplace_levitate\mock_config_storage;
use contentmarketplace_levitate\mock_curl;
use contentmarketplace_levitate\model\learning_object;
use contentmarketplace_levitate\oauth;
use contentmarketplace_levitate\oauth_rest_client;
...
class generator extends component_generator implements learning_object_generator, config_setup_generator {
    ...
    /**
     * Set up an access token so that the mock curl client will be used instead of actually polling the levitate API.
     */
    public function set_up_configuration(): void {
        set_config('oauth_access_token', '--ACCESS-TOKEN--', 'contentmarketplace_levitate');
    }
    ...
    public function get_mock_config_storage(bool $with_access_token = true): mock_config_storage {
        global $CFG;
        require_once("$CFG->dirroot/totara/contentmarketplace/contentmarketplaces/levitate/tests/fixtures/mock_config_storage.php");
    ...
    public function get_mock_rest_client(string $endpoint = api::ENDPOINT): oauth_rest_client {
        global $CFG;
        require_once("$CFG->dirroot/totara/contentmarketplace/contentmarketplaces/levitate/tests/fixtures/mock_curl.php");
    ...
    public function get_mock_learning_object(int $id): array {
        global $CFG;
        $path = "/totara/contentmarketplace/contentmarketplaces/levitate/tests/behat/fixtures/learning-objects/$id/GET.json";

        $mock_file = @file_get_contents($CFG->dirroot . $path);
        if (!$mock_file) {
            throw new coding_exception("Must specify the ID of a mocked levitate learning object that exists at $path");
        }
     ...
     *                                        totara/contentmarketplace/contentmarketplaces/levitate/tests/behat/fixtures/learning-object
     * @return model
     */
    public function generate_learning_object(?string $learning_object_id = null): model {
        if (empty($learning_object_id) || !is_numeric($learning_object_id)) {
            throw new coding_exception(
                "Must specify an ID of a mocked levitate learning object and not a name ('$learning_object_id' was specified)"
            );
        }
