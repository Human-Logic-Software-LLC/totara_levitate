...
final class string_manager {
        ...
        if (\core_text::strpos($lang, '-') > 0) {
            ...
            if (array_key_exists($langcode, $this->languages)) {
                ...
                if ($this->manager->string_exists($countrycode, 'core_countries')) {
                    ...
                    return get_string('langwithcode', 'contentmarketplace_levitate', $a);
                }
            }
        }
        if (empty($lang)) {
            return get_string('unknownlanguage', 'contentmarketplace_levitate');
        }
        return $lang;
    }
    ...
    public function get_region($region) {
        ...
        if ($this->manager->string_exists($identifier, 'contentmarketplace_levitate')) {
            return get_string($identifier, 'contentmarketplace_levitate');
        }
        return $region;
    }
