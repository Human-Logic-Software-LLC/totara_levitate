...
function contentmarketplace_levitate_create_course_module_source_records(): void {
    global $DB;
    ...
    $DB->execute("
        INSERT INTO {marketplace_levitate_learning_object} (external_id)
        SELECT {$source_substring}
        FROM (
            SELECT DISTINCT source
            FROM {files} files
            LEFT JOIN {marketplace_levitate_learning_object} learning_object ON source = {$learning_object_field}
    ...
    $DB->execute("
        ...
        SELECT context.instanceid, learning_object.id, 'contentmarketplace_levitate'
        FROM {marketplace_levitate_learning_object} learning_object
        ...
        LEFT JOIN {totara_contentmarketplace_course_module_source} cm_source
          ...
          AND cm_source.marketplace_component = 'contentmarketplace_levitate'
